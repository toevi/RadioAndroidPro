...
- **Why LibVLC and Not ExoPlayer (sound quality, real 10-band EQ, and the metadata trade-off)**
...
- **Serial VLC Command Thread — blocking libvlc calls off the main thread (ANR / `vlc_join`)**
...
| 🪟 Windows 10/11 (x64) | **Port done** — same C# / .NET MAUI codebase with the LibVLC engine: 10-band equalizer, ICY metadata, cover art, skins, sleep timer. The Android car and phone layer (Android Auto, Bluetooth media session, cellular fallback) does not apply on the desktop. **Installer coming soon.** |
...
## 🔊 Why LibVLC and Not ExoPlayer

The obvious choice for an Android audio app is **ExoPlayer** (Media3). It is Google's own player, it is written for Android, and its Android Auto / MediaSession / metadata integration is first-class and well-documented. Choosing it would have made most of this README unnecessary — no native-thread deadlocks, no SIGSEGV cleanup rules, no `vlc_join` ANRs. So why fight LibVLC instead?

**Two reasons, and both are deliberate trade-offs — harder to build, better to listen to.**

### 1. Sound quality and a real 10-band equalizer

This is the whole point of the app. LibVLC decodes and renders audio through the same battle-tested engine that powers desktop VLC, and — crucially — it exposes a **true 10-band parametric equalizer** that operates on the decoded audio signal itself. The bands do what they say: real gain applied at real frequencies, the same DSP a desktop media player uses.

ExoPlayer's built-in options are, by comparison, a **cosmetic "sound enhancer"** — presets and loudness tricks layered on top, not genuine per-band control of the signal. On paper both "have an equalizer." In the ear they are not the same thing. On good headphones or a real car system the difference is **audible and tangible** — which is exactly the audience this app is built for.

Everything hard in this README — Android Auto, Bluetooth, MediaSession, foreground-service timing, native memory safety — is the price paid for that engine. It is a price worth paying, because the payoff is the one thing a radio app is actually *for*: how it sounds.

### 2. VLC treats metadata as an afterthought — so the app builds its own

Here the trade-off runs the other way, and it is the hidden cost nobody warns you about.

ExoPlayer treats stream metadata as a **first-class feature**: ICY titles, timed metadata, artist/title parsing arrive through a clean, documented API. LibVLC treats metadata as a **side dish**. It is built to *play* the stream; pulling artist, title, and cover art out of it is best-effort and inconsistent. For some streams VLC surfaces the ICY title cleanly; for many others it exposes nothing usable at all, or exposes it late, or changes it without notifying reliably.

That gap had to be filled by hand, with **two cooperating subsystems**:

- **A layer that normalizes what LibVLC *does* give us** — reconciling VLC's inconsistent metadata events into a single, stable "now playing" state the UI, notification, Android Auto, and Bluetooth session can all trust, without the flicker and stale-value problems VLC's raw events produce.
- **A second, independent layer that extracts metadata VLC cannot** — reaching into the stream directly to recover the station/track information LibVLC leaves on the table, and (for the cover-art feature) resolving artwork separately rather than depending on the engine to provide it.

This is the inverse of the ExoPlayer world: there, playback is hard-won and metadata is free; here, **the sound is world-class and the metadata is the part you have to engineer yourself.** The metadata deep-dives later in this document (`§7 AAOS Album Art`, and the AVRCP metadata section) are downstream of exactly this decision.

> **The summary:** ExoPlayer would have been easier everywhere except the two things that matter most for *this* app — the quality of the sound and the fidelity of the equalizer. LibVLC wins those decisively. The cost is that platform integration and metadata both had to be built from scratch. That cost is what the rest of this README documents.
...
**Per-stream options** (applied to each `Media` object before `Play()`) — **selected per protocol**, because HLS and ICY/Shoutcast fail in different ways. The URL is sniffed for `.m3u8` (`IsHlsUrl()`); everything else gets the ICY preset.

| Preset | Options | Why |
|---|---|---|
| **HLS** (`.m3u8`) | `:network-caching=10000` · `:live-caching=10000` · `:http-reconnect` · `:adaptive-logic=rate` | 10 s buffers ride out a multi-second cell-tower handover gap. `adaptive-logic=rate` picks the variant matching *measured* bandwidth, so VLC **downshifts** instead of starving — `=highest` used to force the top variant and stall on the highway. |
| **ICY / Shoutcast** (everything else) | `:network-caching=7000` · `:live-caching=7000` · `:http-reconnect` | 7 s buffer survives short cellular gaps; extra latency is irrelevant for live radio, and ICY reconnects cleanly on its own. |
...
> **Why the buffers were trimmed −15 % (HLS 12→10 s, ICY 8→7 s).** The original oversized buffers were a crutch for the lack of self-recovery. Once **FIX 18** added real recovery (bounded Stop + watchdog + reconnect — see Technical Deep Dive §12), the buffer no longer has to survive every gap on its own, so it was shortened to cut startup latency. A huge buffer only delays the first sound; it does not fix a stream that has actually wedged.
...
- **Serial command thread** — every control command (`Play`/`Stop`/`Pause`/`HardReset`/station switch) runs on one dedicated background thread, never the UI thread. `Stop()` blocks synchronously in `vlc_join`; on the main thread that is an ANR (see §11).
- **Bounded Stop / Dispose** — on a stalled HLS stream `Stop()`/`Dispose()` can block forever in a stuck network read. `StopPlayerBounded` (6 s) and `TryDisposeBounded` (3 s) abandon the wedged player, recreate fresh LibVLC/MediaPlayer instances immediately, and dispose the stuck ones 60 s later — so one wedged call never freezes the app (see §12).
...
**The recommended pattern — two-level browse hierarchy**

> **⚠️ Note — what actually ships vs. what is recommended.** The current production `OnLoadChildren`
> (`AudioPlaybackService.cs`) returns stations as a **flat list directly under `__ROOT__`** — i.e. the
> "wrong pattern" above. This is a deliberate, accepted trade-off: the app ships with a curated
> station list, the flat list is simpler, and the favorites filter keeps the visible set small. The
> known limitation is that on head units that page at 10 items, stations beyond the 10th are
> truncated in the Android Auto browser. The two-level hierarchy described below is the **recommended
> upgrade for large lists**, not the code as currently shipped. If you maintain a list longer than 10
> stations, apply it.
...
**Blocking-call rules — never hang the main thread or the command queue** (§11–§12)

| Rule | Why |
|---|---|
| Route every control command (`Play`/`Stop`/`Pause`/`HardReset`/station switch) through `RunOnVlcThread(...)` | `Stop()` blocks in `vlc_join` until the input thread exits — on the main thread that is an ANR |
| Call `Stop()` only via `StopPlayerBounded()` (6 s cap); on timeout abandon the player and recreate fresh instances | A stuck HLS network read makes `Stop()` block forever and wedges the single command thread |
| Call hard-reset `Dispose()` only via `TryDisposeBounded()` (3 s cap) | `Dispose()` blocks on the same stuck input thread — leak the instance rather than hang the queue |
| Never feed `Buffering` events to the watchdog (`MarkVlcActivity`) — only `TimeChanged` / `PositionChanged` | A stalled HLS stream emits endless `Buffering` with no progress, starving the watchdog exactly when recovery is needed |
...
| App unresponsive on cellular during an HLS station, buttons dead | `Stop()` blocked in `vlc_join` on the command thread — needs bounded Stop | §11 / §12 |
| ANR at startup / after a system font-size change | Blocking libvlc call on the main thread, or `FontScale` config change | §11 / §14 |
| Blank text on an older car head unit over Bluetooth | Oversized album-art bitmap dropped the whole AVRCP metadata packet | §13 |

---

### 11. Blocking libvlc Calls on the Main Thread → ANR (the Serial Command Thread)

**The symptom**

The app freezes and Android shows the "RadioAndroid isn't responding" ANR dialog. The trace points at the **main (UI) thread**, stuck deep inside native VLC code — typically `input_Close` / `vlc_join`. It happens on a station switch, a Stop, or a hard reset, and almost always when the network is bad.

**The root cause**

`libvlc_media_player_stop()` is **synchronous**. It does not return until the VLC input thread has actually exited — and that thread may be blocked in a socket read waiting for data that will never come (dead cell, dropped server). On a good network the call returns in milliseconds, so during desktop/Wi-Fi testing it looks instant. On a dying network it blocks for **many seconds**.

Section §1 covers the *native-thread deadlock* (calling VLC from inside a VLC event callback). This is a different failure with the same root discipline: here the danger is calling a **blocking** VLC method (`Stop()`, `Dispose()`) from the **Android main thread**. The main thread is where Android draws frames and dispatches input; block it for ~5 s and the system raises an ANR and may kill the app.

**The fix — one dedicated serial command thread**

Every VLC control command (`Play`, `Stop`, `Pause`, `HardReset`, station switch) is funneled through a single background thread — `AudioPlaybackService.CommandQueue.cs`, the 9th partial of the service:

```csharp
// A BlockingCollection<Action> consumed by one long-lived background thread.
private static readonly BlockingCollection<Action> _vlcCommandQueue = new();
private static Thread? _vlcCommandThread;   // Name = "VlcCommandThread", IsBackground

private static void RunOnVlcThread(Action action)
{
    EnsureVlcCommandThread();
    _vlcCommandQueue.Add(action);   // enqueue; the loop drains it in order
}
```

This buys **two** guarantees at once:

1. **The main thread never blocks in VLC.** All the slow, synchronous native calls run on `VlcCommandThread`, not on the UI thread. No more `vlc_join` ANRs.
2. **Commands are serialized.** Because there is exactly one consumer thread, no two threads ever drive the native player concurrently. A Stop can never interleave with a Play mid-flight — the ordering problem that used to corrupt player state simply cannot occur.

> **Rule:** never call `Play()`, `Stop()`, `Pause()`, `HardReset()`, or a station switch directly. Always go through `RunOnVlcThread(...)`. The UI thread's job is to *enqueue intent*, never to touch the native engine.

This thread is the foundation for §12 — but note it is not by itself sufficient. A single serial thread means one wedged command blocks *every* command behind it. §12 is what stops that from happening.

---

### 12. HLS Packet-Loss Freeze — Bounded Stop / Dispose & Wedged-Player Recovery (FIX 18)

**The symptom**

Playing an HLS (`.m3u8`) station in a moving car — tower handovers, weak LTE — the audio goes silent and the app stops responding to **everything**: on-screen buttons, Android Auto controls, Bluetooth steering-wheel buttons. No sound, no recovery, no crash. Only a force-stop brings it back.

**The root cause — two gaps left open even with the command thread**

When HLS loses segments, VLC's input thread gets stuck in a blocking network read. That exposes two holes:

1. **An unbounded `Stop()` wedges the *one* command thread.** With everything serialized onto a single `VlcCommandThread` (§11), a `Stop()` that blocks forever in `vlc_join` also blocks *every command queued behind it* — the next Play, the reconnect, the watchdog's own reset, the Android Auto button press. The serialization that makes §11 safe is exactly what turns one stuck call into a total freeze. This is the primary symptom.
2. **The watchdog was being fed by `Buffering` events.** A stalled HLS stream emits `Buffering` **endlessly** with no forward time progress. The watchdog's `MarkVlcActivity()` was called on `Buffering`, so the "no activity for 15 s" timer kept resetting and the watchdog **never fired** — killing the one mechanism that should have recovered automatically.

**The fix (FIX 18)**

| Part | What it does |
|---|---|
| **`StopPlayerBounded()`** | Runs `Stop()` on a worker task with a hard **6 s** cap (`StopVlcTimeoutMs`). If it returns in time, normal path. If it doesn't, the player is **abandoned** — not disposed (Dispose would block on the same stuck thread). |
| **`RecoverFromWedgedPlayer()`** | Immediately builds **fresh** `LibVLC` + `MediaPlayer` instances so the command thread and playback recover at once. The wedged instances are disposed **60 s later** on a detached task — by then the stuck socket has usually hit its own timeout and released the input thread, so `Dispose()` completes without blocking anything. A 60-second native leak is far cheaper than a dead command queue. |
| **`TryDisposeBounded()`** | The `Dispose()` calls in `HardResetVlc` also get a hard cap (3 s); a wedged instance is leaked rather than allowed to hang the command thread. |
| **Watchdog no longer trusts `Buffering`** | Only `TimeChanged` / `PositionChanged` (real audio progress) feed `MarkVlcActivity()`. A frozen HLS stream now goes quiet on the watchdog → it fires after 15 s → hard reset + reconnect. |

```csharp
private bool StopPlayerBounded(string caller)
{
    var player = MediaPlayerInstance;
    if (player == null) return false;

    var stopTask = Task.Run(() => { try { player.Stop(); } catch { } });
    if (stopTask.Wait(StopVlcTimeoutMs))   // 6 s cap
        return false;                       // completed normally

    // Stop() is still blocked in a stuck network read — abandon the player,
    // build a fresh one, dispose the wedged instances much later.
    RecoverFromWedgedPlayer(player);
    return true;
}
```

Used everywhere a Stop happens: `PlayRadio` (safe-reset before a new station), `StopRadio`, `HardResetVlc`, `OnDestroy`.

> **Rule:** no libvlc call that can wait on the input thread (`Stop()`, `Dispose()`) may run without a time bound — *even on the command thread*. Bound it, and on timeout abandon the instance and recreate a fresh one. And never let an event that fires with no real progress (`Buffering`) feed the watchdog, or the watchdog starves exactly when it is needed most.

---

### 13. Bluetooth AVRCP Metadata — Blank Display on Older Car Head Units (BT_FIX)

**The symptom**

On an older car head unit (Bluetooth **AVRCP 1.3**, ~2014) RadioAndroid showed a **blank display** — no station name, no track title, just a generic music glyph. A competing app (VRadio) showed full text on the *same* unit. That was the proof: the missing text is **app-side, not a hardware limit**.

**The root cause**

`UpdateNowPlaying` (`AudioPlaybackService.Media.cs`) put a **full-size album-art bitmap** into `MediaMetadataCompat` with no scaling — iTunes/Deezer covers are often 600×600 or larger. Session metadata is delivered to the Bluetooth AVRCP service **over Binder**, and Binder has a per-transaction size limit. On an old AVRCP 1.3 stack an oversized bitmap can exceed that limit, and when it does the head unit drops the **entire metadata packet — including the station name and track text.** The phone and Android Auto use a different path with a larger limit, so the problem was invisible in every test that didn't involve that specific car. VRadio sent only a tiny glyph, so its (small) packet always got through — hence its text appeared.

Two separate things, only one of them fixable:

| Element | Depends on | Fixable in-app? |
|---|---|---|
| **Album artwork** (the image) | Hardware — needs AVRCP **1.6** (BIP/cover-art); this unit is 1.3 | ❌ No |
| **Text** (station name, track) | App — delivered via MediaSession metadata | ✅ Yes |

**The fix (BT_FIX)**

| # | Change |
|---|---|
| 1 | **`ScaleForSession()`** — every bitmap entering session metadata is scaled to a max **320 px** long edge before `SetMetadata`. Applied to fetched cover art (`FetchAndApplyAlbumArtAsync`, `TryUpdateNowPlayingArtAsync`) and the local fallback (`GetFallbackArt`). Android Auto / AVRCP 1.6 units still get a sharp cover at this size; the metadata packet is now small enough that the **text always reaches** AVRCP 1.3. |
| 2 | **`MetadataKeyAlbum` = plain station name** (no `★` glyph — an older head-unit font may render `★` as tofu). Many AVRCP 1.3 units render the Album line, giving one more reliable text field. |

> **Rule:** never put a full-size bitmap into `MediaMetadataCompat`. Scale every session bitmap to ~320 px before `SetMetadata`, or an old AVRCP 1.3 head unit will drop the whole metadata packet — text included — on the Binder transaction limit.

This cannot be verified on an emulator (no AVRCP over the emulated USB link) — only on real hardware. It was confirmed over Bluetooth on a TV: full text plus the scaled Deezer cover came through intact.

---

### 14. System Font-Scale ANR — the `AttachBaseContext` Override

**The symptom**

The app freezes / ANRs when the **system font size** is changed while it is running (or launches into a bad state on a device with a non-default font scale).

**The root cause**

`FontScale` is **not** listed in the activity's `ConfigurationChanges`, so a system font-size change forces Android to tear down and recreate the activity. Reading the scaled configuration back through the live `Resources` getter during that window can stall the main thread. This is the same class of problem as the others: heavy work landing on the UI thread at the wrong moment.

**The fix**

Neutralize the font scale in `AttachBaseContext` — before the activity's context is fully built — instead of reacting to it later through `Resources`:

```csharp
protected override void AttachBaseContext(Context? @base)
{
    // FontScale isn't in ConfigurationChanges, so pin it to 1.0 at the base
    // context rather than reading it back through the live Resources getter.
    if (@base?.Resources?.Configuration is { } baseConfig && baseConfig.FontScale != 1.0f)
    {
        var cfg = new Configuration(baseConfig) { FontScale = 1.0f };
...
Pinning the scale at the base context keeps the app's own layout stable and removes the main-thread stall. (The app already sizes its own text; it does not need to inherit the system multiplier.)
...
| **Serial Command Thread** | Main-thread ANR — all blocking libvlc calls (`Stop`/`Play`/`HardReset`) run on one `VlcCommandThread`, never the UI thread; also serializes command ordering (§11) |
| **Bounded Stop / Dispose** | HLS packet-loss freeze — `StopPlayerBounded` (6 s) / `TryDisposeBounded` (3 s) abandon a wedged player and recreate a fresh one instead of hanging the command queue (§12) |
...
| **Scaled Session Art** | Blank Bluetooth display — session bitmaps scaled to 320 px so the AVRCP metadata packet fits the Binder limit (§13) |
...
│   ├── AudioPlaybackService.CommandQueue.cs — Serial VLC command thread (RunOnVlcThread)
...
The `AudioPlaybackService` is a `partial class` split across 9 files by responsibility. This keeps each file focused and manageable while sharing state through the service instance.

| File | Responsibility |
|---|---|
| `AudioPlaybackService.cs` | Service lifecycle, `OnCreate`, `OnStartCommand`, `OnDestroy`, `OnLoadChildren` |
| `AudioPlaybackService.Playback.cs` | `PlayRadio`, `StopRadio`, `PauseRadio`, `HardResetVlc`, equalizer, bounded Stop/Dispose (§12) |
| `AudioPlaybackService.Reconnect.cs` | Reconnect loop, exponential backoff, connectivity events |
| `AudioPlaybackService.Media.cs` | VLC event handlers, metadata polling, notifications, session-art scaling (§13) |
...
| `AudioPlaybackService.Watchdog.cs` | Watchdog timer, VLC activity tracking (`Buffering` excluded — §12) |
| `AudioPlaybackService.CommandQueue.cs` | Serial VLC command thread — `RunOnVlcThread`, keeps blocking libvlc calls off the main thread (§11) |
